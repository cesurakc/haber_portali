...
public class vtIsletimcisi<T> implements Icrud<T>{
    ...
    /**
     * 
     * @param silinecekid NOTTUR.. Sil id sini 0 a çekerek düzenlenecek.
     * @return 
     */
    @Override
    ...
    public List<T> listele(T t) {
        ...
        try{
        sessionAC();
        String hql=" from tblhaber where kategoriid=66";
    ...
    public List<T> listeslider(T t) {
		 List<T> listem;
	        try{
	        sessionAC();
	        String hql = "FROM tblhaber order by id desc";
	        Query query = ss.createQuery(hql);
    ...
    public List<T> listB(T t) {
		  ...
		  Session ss1 = hibernateutil.getSessionFactory().openSession();
	        ...
	        sessionAC();
	        Criteria cr = ss1.createCriteria(t.getClass());
	        cr.add(Restrictions.eq("sil_id", 0));
	        listem = cr.list();
	        sessionKapat();
	        return listem;
	        }catch(Exception ex){
	...
	public void KategoriGuncelleme(int id, String ad,String aciklama) {
		...
		try {
			sessionAC();
			tblkategori kat = ss.get(tblkategori.class, id);
			kat.setId(id);
			kat.setAd(ad);
			kat.setAciklama(aciklama);
			...
			ss.update(kat);
			 ...
			 sessionKapat();
			  ...
			  tbllog log = new tbllog();
	          log.setSinif(tblkategori.class.toString());
	          log.setMethod("Listele");
	          Date dt = new Date();
	          log.setTarih(dt);
	          log.setHataicerigi(ex.toString());
	...
	public void HaberGuncelle(int id, String baslik, String ozet, String tarih, String resimurl) {
		...
		try {
			sessionAC();
			tblhaber tbh=ss.get(tblhaber.class, id);
			tbh.setId(id);
			tbh.setBaslik(baslik);
			tbh.setOzet(ozet);
			tbh.setTarih(tarih);
			tbh.setResimurl(resimurl);
	...
	public void KullaniciGuncelle(int id, String ad, String sifre) {
		...
		try {
			sessionAC();
			tblusers tbl=ss.get(tblusers.class, id);
			tbl.setId(id);
			tbl.setAd(ad);
			tbl.setSifre(sifre);
